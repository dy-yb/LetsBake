...
class ExchangerViewModel {
  ...
  func calculator(inputIngredient: String, inputUnit: String, inputQuantity: Double, resultUnit: String) -> Double {
    var resultQuantity: Double = 0

    for index in 0 ... ingredientsRatio.count-1 {
      if inputIngredient == ingredientsRatio[index].ingredient {
        let ratio: Units = ingredientsRatio[index].quantityAsUnits
        if inputUnit != "gram" {
